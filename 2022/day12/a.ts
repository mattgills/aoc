...
queue.push({ x: start[1], y: start[0], level: 0});
...
while (queue.length > 0) {
    ...
    if (y > 0 && graph[y - 1][x] - currHeight <= 1) {
        queue.push({ x, y: y - 1, level: nextLevel });
    }

    if (y < graph.length - 1 && graph[y + 1][x] - currHeight <= 1) {
        queue.push({ x, y: y + 1, level: nextLevel });
    }

    if (x > 0 && graph[y][x - 1] - currHeight <= 1) {
        queue.push({ x: x - 1, y, level: nextLevel });
    }

    if (x < graph[0].length - 1 && graph[y][x + 1] - currHeight <= 1) {
        queue.push({ x: x + 1, y, level: nextLevel });
    }
}
